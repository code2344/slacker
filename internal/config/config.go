...
type (
	...
	SidebarConfig struct {
		// Visible controls whether the conversations sidebar is shown by default.
		// It can still be toggled at runtime with the toggle_conversations_tree keybind.
		Visible bool `toml:"visible"`
		// WidthPercent is the percentage of the window used by the conversations sidebar.
